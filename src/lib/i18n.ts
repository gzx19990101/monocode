...
const ZH: Record<string, string> = {
  ...
  "Agent CLIs MonoCode can drive, and the model new sessions start with.":
    "MonoCode 可以驱动的智能体 CLI，以及新会话使用的默认模型。",
  ...
  "Show the current effort as a separate control beside the model picker for quicker changes. When off, effort stays inside the model menu.":
    "在模型选择器旁显示当前推理力度的独立控件，便于快速调整。关闭时，力度选项保留在模型菜单内。",
  "Show effort beside model picker": "在模型选择器旁显示推理力度",
  "Code review": "代码审查",
